...
using Nethermind.State.RetrieveStorage;
...
namespace Nethermind.JsonRpc.Modules.Eth
{
    public partial class EthRpcModule : IEthRpcModule
    {
        ...
        public ResultWrapper<string> eth_getContractStorage(Address accountAddress, BlockParameter blockParameter)
        {
            BlockHeader header;
            try
            {
                ...
                if (header == null)
                {
                    return ResultWrapper<string>.Fail("Header not found", ErrorCodes.ResourceNotFound, null);
                }

                if (!HasStateForBlock(_blockchainBridge, header))
                {
                    return ResultWrapper<string>.Fail("State not found", ErrorCodes.ResourceUnavailable, null);
                }
            }
            catch (Exception ex)
            {
                return ResultWrapper<string>.Fail(ex.Message, ErrorCodes.InternalError, null);
            }

            Retriever retriever = new(accountAddress);
            _blockchainBridge.RunTreeVisitor(retriever, header.StateRoot);

            return ResultWrapper<string>.Success(retriever.BuildResult());
        }
